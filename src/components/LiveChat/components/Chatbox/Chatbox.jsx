  ...
  const clickHandler = async () => {
    ...
    if (inputMsg || files?.length > 0) {
      await socket.current.emit(
        ...
        cookies.chat_session_id, teamCdn,
        supportMsgId
      );
    }
    setInputMsg("");
        ...
        {allMessages.map((msg) => {
          return (
            (msg?.content || msg?.Support_Chat_Attachments?.length > 0) && (
              <div
                className={
                  styles.msgContainerLeft +
                  " " +
                  (cookies.chat_user_id === msg.user?.chat_user_id &&
                    styles.msgContainerRight)
                }
              >
                <div
                  className={
                            ...
                            {attachment?.attachment_type === "audio/wav" ? (
                              ...
                              >
                                ...
                                <div className={styles.attachemntText}>
                                  ...
                                  <p>
                                    {formatBytes(attachment.attachment_size)}
                                  </p>
                                </div>
                                {/* {fetchingFile ===
                ...
                <p className={styles.msgInfo}>
                  {moment(msg?.created_at).format("Do MMMM YYYY, h:mm a")}
                </p>
              </div>
            )
          );
      ...
      <div className={styles.attachment_name}>
        {files?.length > 0 &&
          (uploadingMultimedia ? (
            ...
            supportMsgId && (
              <div className={styles.images_name}>{files[0].name}</div>
            )
          ))}
      </div>
      <footer>
